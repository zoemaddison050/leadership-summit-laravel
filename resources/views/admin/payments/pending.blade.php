    ...
    <div class="page-actions">
        <div class="btn-group">
            ...
            <ul class="dropdown-menu">
                <li><a class="dropdown-item" href="{{ route('admin.payments.pending') }}">All Pending</a></li>
                <li><a class="dropdown-item" href="{{ route('admin.payments.pending', ['filter' => 'today']) }}">Today Only</a></li>
                <li><a class="dropdown-item" href="{{ route('admin.payments.pending', ['filter' => 'week']) }}">This Week</a></li>
            </ul>
        </div>
    </div>
</div>
...
<div class="stats-grid mb-4">
    <div class="stat-card warning">
        ...
        <div class="stat-change">
            <span>{{ $stats['pending_today'] }} today</span>
        </div>
    </div>
    ...
    <div class="stat-card danger">
        ...
        <div class="stat-value">{{ $stats['total_declined'] }}</div>
        <div class="stat-change">
            <span>All time</span>
        </div>
    </div>
...
<div class="admin-card">
    <div class="admin-card-header">
        ...
        <div class="d-flex gap-2">
            <span class="badge bg-warning">{{ $pendingRegistrations->total() }} pending</span>
        </div>
    </div>
    <div class="admin-card-body p-0">
            ...
            <table class="table table-hover mb-0">
                ...
                <tbody>
                    ...
                    <tr>
                        <td>
                            <div class="d-flex align-items-center">
                                ...
                                <div>
                                    ...
                                    <br>
                                    <small class="text-muted">{{ $registration->attendee_phone }}</small>
                                </div>
                            </div>
                        </td>
                        <td>
                            <div>
                                ...
                                <small class="text-muted">{{ $registration->event->start_date->format('M d, Y g:i A') }}</small>
                            </div>
                        </td>
                        <td>
                            <span class="fw-semibold">${{ number_format($registration->total_amount, 2) }}</span>
                            @if($registration->ticket_selections)
                            <br>
                        ...
                        <td>
                            <div>
                                <span class="fw-semibold">{{ $registration->payment_confirmed_at->format('M d, Y') }}</span>
                                <br>
                                <small class="text-muted">{{ $registration->payment_confirmed_at->format('g:i A') }}</small>
                                <br>
                                <small class="text-muted">{{ $registration->payment_confirmed_at->diffForHumans() }}</small>
                            </div>
                        </td>
                        <td>
                        ...
                        <td class="text-end">
                            <div class="btn-group">
                                <button type="button" class="btn btn-sm btn-success"
                                    onclick="approvePayment({{ $registration->id }})"
                                    title="Approve Payment">
                                    <i class="fas fa-check" aria-hidden="true"></i>
                                    Approve
                                </button>
                                <button type="button" class="btn btn-sm btn-danger"
                                    onclick="showDeclineModal({{ $registration->id }}, '{{ $registration->attendee_name }}')"
                                ...
                                <ul class="dropdown-menu dropdown-menu-end">
                                    <li>
                                        <a class="dropdown-item" href="#" onclick="viewRegistrationDetails({{ $registration->id }})">
                                            <i class="fas fa-eye me-2" aria-hidden="true"></i>View Details
                                        </a>
                                    </li>
                                    <li>
                                        <a class="dropdown-item" href="mailto:{{ $registration->attendee_email }}">
        ...
        <div class="d-flex justify-content-between align-items-center p-3 border-top">
            <div class="text-muted">
                Showing {{ $pendingRegistrations->firstItem() }} to {{ $pendingRegistrations->lastItem() }}
                of {{ $pendingRegistrations->total() }} results
            </div>
