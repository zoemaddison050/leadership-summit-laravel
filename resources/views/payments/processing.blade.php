...
@section('title', 'Processing Payment - ' . $event->title)
...
<div class="processing-container">
    <div class="processing-card">
        ...
        <p class="processing-message">
            @if(isset($registrationData['payment_method']) && $registrationData['payment_method'] === 'card')
        ...
        <div class="event-info">
            <h4>{{ $event->title }}</h4>
            <p>Total Amount: ${{ number_format($registrationData['total_amount'], 2) }}</p>
            <p>Email: {{ $registrationData['attendee_email'] }}</p>
            @if(isset($registrationData['payment_method']))
            <p>
                Payment Method:
                @if($registrationData['payment_method'] === 'card')
                <i class="fas fa-credit-card me-1"></i>Credit/Debit Card
                @elseif($registrationData['payment_method'] === 'crypto')
                <i class="fas fa-coins me-1"></i>Cryptocurrency
                @else
                {{ ucfirst($registrationData['payment_method']) }}
                @endif
            </p>
            @endif
        </div>
        ...
        <a href="{{ route('home') }}" class="manual-redirect-btn" id="manualRedirectBtn">
...
<script>
    ...
    function processPaymentConfirmation() {
        ...
        fetch('{{ route("payment.process", $event) }}', {
                method: 'POST',
                headers: {
