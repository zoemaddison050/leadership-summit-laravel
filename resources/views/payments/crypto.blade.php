...
@section('title', 'Crypto Payment - ' . $event->title)
    ...
    <div class="container">
        <div class="payment-card">
            ...
            <div class="order-summary">
                ...
                @if(!empty($registrationData['attendee_phone']))
                <div class="summary-item">
                    <div>
                ...
                @if(!empty($registrationData['emergency_contact_name']))
                <div class="summary-item">
                    <div>
                        <div><strong>Emergency Contact:</strong> {{ $registrationData['emergency_contact_name'] }}</div>
                        @if(!empty($registrationData['emergency_contact_phone']))
                ...
                @foreach($ticketSelection['tickets'] as $ticketData)
                <div class="summary-item">
                    <div>
                        <div>{{ $ticketData['ticket']->name }}</div>
                        <small class="text-muted">Qty: {{ $ticketData['quantity'] }} × ${{ number_format($ticketData['ticket']->price, 2) }}</small>
                    </div>
                    <span>${{ number_format($ticketData['subtotal'], 2) }}</span>
                </div>
                @endforeach
                <div class="summary-item">
                    <span>Total:</span>
                    <span>${{ number_format($ticketSelection['total_amount'], 2) }}</span>
                </div>
                @endif
            </div>
...
<script>
    ...
    window.cryptoPaymentConfig = {
        hasRegistrationData: {{ isset($registrationData) ? 'true' : 'false' }},        csrfToken: '{{ csrf_token() }}',
        detailsUrl: '{{ route("payment.crypto.details", $event) }}',
        confirmUrl: '{{ route("payment.confirm", $event) }}',
        @if(isset($registrationData) && array_key_exists('expires_at', $registrationData))
        registrationExpiresAt: '{{ $registrationData["expires_at"] }}'
        @else
