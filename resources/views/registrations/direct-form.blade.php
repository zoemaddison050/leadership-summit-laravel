...
<div class="registration-container">
    <div class="container">
        <div class="registration-card">
            ...
            <form method="POST" action="{{ route('events.register.process', $event) }}" id="registrationForm">
                @csrf
                        ...
                        <div class="col-md-6">
                            <div class="form-group">
                                ...
                                <input type="text" class="form-control @error('attendee_name') is-invalid @enderror"
                                    id="attendee_name" name="attendee_name" value="{{ old('attendee_name') }}"
                        ...
                        <div class="col-md-6">
                            <div class="form-group">
                                ...
                                <input type="text" class="form-control event-datetime-readonly"
                                    ...
                                    value="{{ $event->start_date->format('l, F j, Y \a\t g:i A') }}"
                                    readonly>
                            </div>
                        </div>
                    </div>
                ...
                <!-- Ticket Selection -->
                <div class="form-section">
                    <h4>{{ $tickets->count() === 1 ? 'Selected Ticket' : 'Ticket Selection' }}</h4>

                    <div class="ticket-selection">
                        @foreach($tickets as $ticket)
                        <div class="ticket-option" data-ticket-id="{{ $ticket->id }}" data-price="{{ $ticket->price }}">
                            <div class="d-flex justify-content-between align-items-start">
                                <div>
                                    <div class="ticket-name">{{ $ticket->name }}</div>
                                    @if($ticket->description)
                                    <div class="text-muted mb-2">{{ $ticket->description }}</div>
                                    @endif
                                ...
                                <div class="quantity-controls">
                                    <button type="button" class="quantity-btn" onclick="changeQuantity({{ $ticket->id }}, -1)">-</button>
                                    <input type="number" class="quantity-input" id="quantity_{{ $ticket->id }}"
                                        value="0" min="0" max="{{ $ticket->quantity ? $ticket->available : 10 }}"
                        ...
                        <div id="ticket-selections-container">
                            @foreach($tickets as $ticket)
                            <input type="hidden" name="ticket_selections[{{ $ticket->id }}]" id="ticket_selection_{{ $ticket->id }}" value="0">
                            @endforeach
                        </div>

                        @error('ticket_selections')
